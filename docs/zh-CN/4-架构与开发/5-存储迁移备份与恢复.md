...
| Electron `userData`  | `window-state.json`、更新缓存等                                                                                     | 主进程数据                 |
...
`create_schema` 当前流程直接建立 29 张表，再由 `image_library::ensure_image_library_table` 建立图像库表，总计 **30 张核心业务表（含 `image_library`）**。`codebase_embed_sessions` 等辅助表和代码库索引的按项目动态表不属于这 29 张直接核心表。
...
5. 按需要复制窗口状态、更新缓存或 SSH 相关数据；凭据必须加密保存并限制访问。
6. 记录应用版本、操作系统和自定义路径，方便恢复时判断兼容性。
