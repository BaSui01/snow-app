...
- **`## Language Servers`** (LSP domain, `build_system_prompt_section` in `native/src/mcp/servers/lsp/mod.rs`): injected when the project has usable external language servers (config enabled + command installed + **a real language-stack marker exists inside the project**, stack-aware since 2026-09-24) and the domain scope allows it. It lists the servers with their runtime state (running / crashed / not started), groups the preferred `lsp-*` tools by capability, and steers semantic queries to `lsp-*` through **scenario-to-tool hard bindings** in the routing rules (grep stays for literal text only); it also prewarms not-yet-running servers in the background using the real **stack root** as the session root, removing first-call cold starts.
...
- **Investigation-phase tool list (template-level dynamic injection, 2026-09-24)**: `native/src/prompt/tool_hints.rs` builds the list from tools **actually callable** in the current project - `lsp-*` (domain scope + usable servers + a matching language stack, the same decision as tool exposure) and `codebase-search` (index available) appear conditionally, while `grep-search` / `filesystem-read` are the constant read-only baseline; at request-build time it replaces the `__ANALYSIS_TOOLS_LINES__` placeholder of the **Plan / Goal / WorkFlow** mode templates (Plan "analysis", Goal "Phase 1 Investigate", WorkFlow "Step 1 pre-design investigation"), so lines for unavailable tools never exist. Tool descriptions carry an additional conditional injection layer (grep-search counter + `lsp-*` timing triggers) - see the LSP design doc §8.10. Priority hardening (2026-09-25): the `lsp-*` line now reads "use these FIRST for symbols / usages / types / impact", while with LSP active the `grep-search` line becomes "literal strings/patterns ONLY ... cannot tell a real reference from a same-named symbol" and the `codebase-search` line states its concept-level scope (mirroring the §8.10 Routing-rules priority preamble).
- All sections are appended at the **end** of the prompt (state changes only affect the tail, minimizing prompt-cache prefix invalidation); any query failure silently degrades to an empty string (never breaks the request); normal / Plan / Goal modes all inject them, and sub-agent prompts carry them too.
