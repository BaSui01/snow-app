...
| Broken images for `image/`/`upload/` paths in Markdown                                       | The markdown worker's image rule used to proxy only http(s) images, so local relative paths were loaded as relative URLs and broke. The worker now tags such `<img>` with `data-local-image` (`isLocalImagePath` in `markdownWorker.ts` — decodes first, normalizes separators, rejects `..` and absolute paths), and the renderer resolves them to data URLs via `resolveLibraryImage`/`resolveUploadImage` (`resolveLocalImages` in `markdownRenderer.tsx` — module-level cache + in-flight dedupe); keep `isLocalImagePath` and `normalizeLocalImagePath` in sync when adding path types |
