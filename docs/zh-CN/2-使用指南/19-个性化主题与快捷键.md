...
> 本指南说明 Snow App 的规则组合、请求配置、主题、语言和快捷键行为。设置侧栏当前包含 21 个设置页；本文涉及「系统提示词」（设置页 id：`system-prompt-settings`）、「个性化」（`personalization-settings`）、「自定义请求头」（`custom-headers-settings`）、「主题设置」（`theme-settings`）、「快捷键」（`keyboard-shortcuts-settings`）等页面。语言选择位于设置列表下方的独立区域，不是第 22 个设置页。
...
每项快捷键可独立启用、重新绑定和恢复默认，页面也可恢复全部默认键。录制普通动作时按 `Esc` 会取消录制；录制“取消当前会话”时 `Esc` 是有效绑定。
