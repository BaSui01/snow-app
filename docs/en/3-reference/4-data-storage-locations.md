...
| `~/.snowapp/plugins/`             | Installed plugin folders (one `<pluginId>` subdirectory per plugin)                                                              | Folders and database rows must be backed up together; `config-delete scope=plugins` with `value={deleteFiles: false}` drops only the registration and keeps the folder |
| `~/.snowapp/docs/`                | Synchronized built-in documentation copy (read by the snow-app-docs skill; `.snow/` belongs to Snow CLI and holds no app assets) | Rebuilt as a whole tree whenever the app version or source-tree fingerprint changes at startup; manual edits are overwritten — restart the app to re-sync              |
...
### 4.1 Platform Differences in Update Caches
...
5. Back up Electron `userData` when needed, especially live browser sessions.
...
5. Verify conversations, images, API profiles, and usage records.
