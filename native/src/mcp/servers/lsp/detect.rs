...
mod tests {
    ...
    /// 在系统临时目录建一个隔离的测试项目（进程 id 防冲突），测试后清理。
    fn tmp_project(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "snow-lsp-detect-test-{name}-{}",
            std::process::id()
        ));
    ...
    /// §8.7.2 项目感知依赖：go.mod / pom.xml 标记能识别 Go/Java 技术栈。
    #[test]
    fn detects_go_and_java_markers() {
        let dir = tmp_project("go-java");
        std::fs::write(dir.join("go.mod"), "module test\n").unwrap();
        std::fs::write(dir.join("pom.xml"), "<project/>\n").unwrap();
        let detected = detect_project_stack(dir.to_str().unwrap());
        let langs: Vec<&str> = detected.iter().map(|d| d.lang.as_str()).collect();
        assert!(langs.contains(&"go"), "go.mod 应识别为 go，实际 {langs:?}");
        assert!(langs.contains(&"java"), "pom.xml 应识别为 java，实际 {langs:?}");
        let _ = std::fs::remove_dir_all(&dir);
    }

    /// 无标记项目 → 空结果（不暴露 type-hierarchy 的保守分支）。
    #[test]
    fn markerless_project_detects_nothing() {
    ...
    /// monorepo：深度 ≤2 的子目录标记也能命中（如 services/api/go.mod）。
    #[test]
    fn detects_stack_in_subdirectory() {
        let dir = tmp_project("monorepo");
        std::fs::create_dir_all(dir.join("services/api")).unwrap();
        std::fs::write(dir.join("services/api/go.mod"), "module api\n").unwrap();
        let detected = detect_project_stack(dir.to_str().unwrap());
        assert!(
            detected.iter().any(|d| d.lang == "go" && d.path == "services/api"),
            "子目录 go.mod 应命中 go，实际 {detected:?}"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
}
