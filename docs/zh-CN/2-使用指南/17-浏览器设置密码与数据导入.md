...
- 点击区块右上角「添加密码」按钮，在弹窗中填写站点（可省略 `https://`）、用户名（可留空）与密码后保存；站点会归一化为 `http(s)://host[:port]` 形式的 origin，自动填充按该 origin 匹配；
- 同一 `origin + username` 再次保存会覆盖旧密码，而不是新增一条；
- 可按站点或用户名搜索；
...
- 自动备份：`~/.snowapp/browser-state/backups/`
