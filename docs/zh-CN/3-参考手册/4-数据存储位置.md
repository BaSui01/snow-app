...
flowchart TB
    ...
    N --> RES[图片 背景 检查点 密码保险库 登录态]
    E --> CH[Chromium 会话 窗口状态]
    G --> CFG[CLI 配置 ROLE Skills 文件日志]
...
| `app_plugins` / `app_plugin_values`                       | 插件注册表（清单字段、启用状态、安装与来源路径）与插件私有持久化值；插件目录位于 `~/.snowapp/plugins/<pluginId>/`               |
...
| `~/.snowapp/browser-state/`       | 登录态存档与 Cookie 自动备份                                                                 | safeStorage 加密；见 2.3 节                                                                                    |
| `~/.snowapp/browser-script/`      | 内置浏览器油猴兼容用户脚本源码文件（`{script_id}.user.js`）                                  | 元数据存 SQLite `userscripts` 表，此处只存源码原文；脚本删除时最佳努力删除文件                                 |
| `~/.snowapp/plugins/`             | 已安装插件目录（每个插件一个 `<pluginId>` 子目录）                                           | 目录与数据库记录需一起备份；`config-delete scope=plugins` 传 `value={deleteFiles: false}` 可只删登记、保留目录 |
| `~/.snowapp/docs/`                | 内置文档同步副本（由 snow-app-docs 技能引导 Agent 阅读；`.snow/` 属 Snow CLI，不放应用资产） | 应用启动时若「应用版本 + 源文档树指纹」变化即整树重建；手工修改会被覆盖，重启应用即可重新同步                  |
...
### 2.3 导出的登录态：`~/.snowapp/browser-state/`
...
恢复前，如果当前浏览器已有 cookies/localStorage，会把同样加密的备份写入 `~/.snowapp/browser-state/backups/`。localStorage 只注入匹配 origin；调试协议不可用时仍可恢复 cookies。

应用还会周期性把内置浏览器的全部 Cookie 以同一加密格式写入 `auto-cookies` 自动备份：macOS Keychain "Chromium Safe Storage" 密钥漂移会让 Chromium 启动时静默清空 Cookie 库，启动检测到数量骤减时自动恢复；用户显式「清除 Cookie」会同步删除该备份，不会误恢复。
...
| `custom-headers.json`              | CLI 自定义请求头同步源，可能含秘密；App 实际生效的请求头方案存于应用数据库 `custom_header_schemes` 表（config 的 `customHeaderSchemes` 域） |
...
| `docs/`                            | 已废弃（内置文档副本已改存 `~/.snowapp/docs/`，此旧目录由应用在下次启动时自动清理）                                                         |
...
5. 按需备份 Electron `userData`，尤其是实时浏览器会话。
