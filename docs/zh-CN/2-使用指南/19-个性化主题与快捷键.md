...
> 本指南说明 Snow App 的规则组合、请求配置、主题、语言和快捷键行为。设置侧栏当前包含 24 个设置页；本文涉及「系统提示词」（设置页 id：`system-prompt-settings`）、「个性化」（`personalization-settings`）、「自定义请求头」（`custom-headers-settings`）、「主题设置」（`theme-settings`）、「快捷键」（`keyboard-shortcuts-settings`）等页面。语言选择位于设置列表下方的独立区域，不是第 25 个设置页。
