...
| Markdown 里 `image/`/`upload/` 图片破图                            | markdown worker 的 image 规则原本只代理 http(s) 外部图，本地相对路径会以相对 URL 加载失败 → 破损图标；worker 已加 `data-local-image` 标记（`markdownWorker.ts` 的 `isLocalImagePath`，先 decode 再统一分隔符，拒绝 `..` 与绝对路径），主线程经 `resolveLibraryImage`/`resolveUploadImage` 解析为 data URL（`markdownRenderer.tsx` 的 `resolveLocalImages`，模块级缓存 + inflight 去重）；新增路径类型需在 `isLocalImagePath` 与 `normalizeLocalImagePath` 两处同步维护 |
