        ...
        vec![McpTool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "prompts": {
                        "type": "array",
                        "description": "DIFFERENT prompt per image (mutually exclusive with `n`): an array of 1-8 prompt strings, one for each image to generate. When provided, the number of requests equals the array length (overrides `n`); each concurrent request uses its own prompt, so you can generate a whole set of DIFFERENT designs (e.g. [\\\"UI mockup: dashboard page\\\", \\\"UI mockup: settings page\\\", \\\"UI mockup: editor page\\\"]) in a single call. Omit it to use the single top-level `prompt` for all images.",
                        "items": { "type": "string" },
                        "minItems": 1,
                        "maxItems": 8
                    },
                    "requestImages": {
                        "type": "array",
                        "description": "DIFFERENT reference image group per request: an array with exactly N groups (N = number of requests = length of `prompts`, or `n`), each group being an array of reference images in the same shape as `images` (see above). Group i is used for request i, enabling image-to-image editing with a DIFFERENT source image per output (e.g. restyle 3 different screenshots in one call). When omitted, all requests share the top-level `images` group.",
                        "items": {
                            "type": "array",
                            "items": {
                                "type": "object",
                    ...
                    "n": {
                        "type": "number",
                        "description": "How many images to generate in one call: 1-8 (default 1). The call internally fans out to n concurrent requests (one image each — the relay/upstream does not accept n>1 in a single request), so all images are produced in parallel and returned together. Streaming preview is disabled when n>1. dall-e-3 always returns exactly 1 image.",
                        "default": 1,
                        "minimum": 1,
                        "maximum": 8
                    },
...
async fn collect_openai_result(
    prompt: &str,
    model: &str,
    channel_label: &str,
    items: Vec<Value>,
