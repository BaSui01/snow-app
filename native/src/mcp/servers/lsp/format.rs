...
mod tests {
    use super::*;
    use lsp_types::{
        CreateFile, DocumentChangeOperation, OptionalVersionedTextDocumentIdentifier, Position,
        Range, ResourceOp, TextDocumentEdit,
    };
    use std::collections::HashMap;

    fn text_edit() -> TextEdit {
        TextEdit {
            range: Range {
    ...
    /// 空 WorkspaceEdit 判定（H2/R1.1）：任意 JSON 对象经 serde 解析（未知字段
    /// 默认忽略）都会得到三字段全 None 的空编辑，必须视为非 WorkspaceEdit。
    #[test]
    fn empty_edit_is_detected() {
        ...
        // serde 反序列化视角：{"status":"ok"} 解析为 Ok(空编辑)。
        let parsed: WorkspaceEdit = serde_json::from_value(json!({"status": "ok"})).unwrap();
        assert!(workspace_edit_is_empty(&parsed));
    ...
    /// Operations 类变更（create/rename/delete file）必须返回明确错误（R1.2）。
    #[test]
    fn workspace_edit_files_operations_is_unsupported() {
        let edit = WorkspaceEdit {
            changes: None,
        ...
        assert!(matches!(
            workspace_edit_files(&edit),
            Err(LspError::Unsupported(_))
        ));
        // 预览路径：标记 unsupportedOperations，不静默返回空列表。
        let preview = workspace_edit_to_value(&edit);
        assert_eq!(preview["unsupportedOperations"], json!(true));
        assert_eq!(preview["changeCount"], json!(0));
    }

    /// `changes` 映射正常提取（回归：不受 Result 改造影响）。
    #[test]
    fn workspace_edit_files_changes_mapping() {
        let edit = WorkspaceEdit {
            changes: Some(HashMap::from([(uri(), vec![text_edit()])])),
            document_changes: None,
            change_annotations: None,
        };
        let files = workspace_edit_files(&edit).unwrap();
        assert_eq!(files.len(), 1);
        assert_eq!(files[0].0, uri());
        assert_eq!(files[0].1.len(), 1);
    }

    /// `document_changes::Edits` 正常提取（含 AnnotatedTextEdit 兼容）。
    #[test]
    fn workspace_edit_files_document_edits() {
        let edit = WorkspaceEdit {
            changes: None,
