...
| 打包中断，报 `gyp: buildcheck.gypi not found`（cpu-features） | 可选依赖的 install 脚本被跳过，gyp 片段缺失 | 见第 6 节 |
...
4. 发布前按第 3 节跑一遍启动验证。
5. 打包报 cpu-features / `buildcheck.gypi` 时，先生成 gyp 片段（第 6 节）；`build.npmRebuild: false` 是兜底开关，引入 nan/V8 ABI 原生依赖时需重新评估。
...
## 6. 案例：原生依赖重建失败导致打包中断（2026-09-15）

### 6.1 现象

`npm run build:win` 在 electron-builder 阶段中断，`release/` 为空：
...
`cpu-features`（`ssh2` 的可选加速依赖，使用 `nan`/V8 ABI，因此需要为 Electron 重建）
在安装时由自己的 install 脚本生成 gyp 片段：

```
"install": "node buildcheck.js > buildcheck.gypi && node-gyp rebuild"
```

该步骤被跳过时（可选依赖安装失败被 npm 忽略，或安装中断），`buildcheck.gypi`
就不存在——它**不在 npm 包的发布文件里**（`npm pack buildcheck@0.0.2` 可核对），
因此重装 `buildcheck` 也无法补齐。

### 6.3 修复

```bash
# 1) 生成 gyp 片段（幂等，可重复执行）
cd node_modules/cpu-features && node buildcheck.js > buildcheck.gypi

# 2) 重新打包（本机需具备 VS C++ 工具链与 Python：@electron/rebuild 会重建
#    cpu-features 与 node-pty）
cd ../.. && npm run build:win
```

若本机确实无法编译原生模块（例如缺少 VS Build Tools 的 C++ 组件），可依赖
`package.json` → `build.npmRebuild: false` 兜底：它会跳过 `@electron/rebuild`。
影响评估：`node-pty` 使用 N-API 且自带 `prebuilds/win32-x64`，无需重建；
`cpu-features` 缺失/未重建时 `ssh2` 自动回退纯 JS（仅性能差异）。
**注意**：将来引入其它 nan/V8 ABI 原生依赖时，必须重新评估并恢复重建。

### 6.4 验证

- 打包日志出现 `finished moduleName=cpu-features arch=x64` 与
  `completed installing native dependencies`；
- 或按第 3 节核对产物（asar 内主进程文件与本地一致、native 绑定位于
