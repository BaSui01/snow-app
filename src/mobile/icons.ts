/**
 * 内嵌 lucide 图标（path 数据与 lucide-react 的图标一致，项目约定图标库只用 lucide）。
 *
 * Mobile 远控页不引入 React，因此这里保留最小化的 path 数据表：
 * - 静态标记用 `data-icon="name"` 属性，启动时 `hydrateIcons()` 水合；
 * - 动态渲染用 `iconMarkup(name)` 生成 SVG 字符串。
 *
 * 新增图标时必须同时在本表与使用处添加，不要保留未使用的条目。
 */
export const ICONS = {
  "panel-left":
  ...
  // 会话列表的类型图标（与桌面 ChatItem 的默认会话图标 / 分支图标一一对应）。
  "message-square-more":
  ...
  // 文件复制/剪切卡片（使用处见 mobile/tools/filesystem.ts 的 renderCopyCard）。
  ...
  // 会话待办图标（状态图标与桌面 TodoPanelButton 的 Circle / CircleDot /
  // CheckCircle2 一一对应，circle-dot 见下方思考强度区）。
  circle: '<circle cx="12" cy="12" r="10"/>',
  "circle-check": '<circle cx="12" cy="12" r="10"/><path d="m9 12 2 2 4-4"/>',
  // 会话列表状态图标（与桌面侧边栏一一对应：需处理 / 已暂停 / 子代理）。
  "circle-alert":
    '<circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="12"/><line x1="12" x2="12.01" y1="16" y2="16"/>',
  pause:
    '<rect x="14" y="3" width="5" height="18" rx="1"/><rect x="5" y="3" width="5" height="18" rx="1"/>',
  bot: '<path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/>',
  // WorkFlow 卡片图标（节点状态：待执行 / 执行中 / 已完成 / 失败，操作按钮）。
  "circle-x":
  ...
  // 检索与代码智能工具卡片（grep 模式 / 代码库检索 / 引用 / 定义 / 重命名），
  // 使用处见 mobile/tools/search.ts。
  regex:
...
export type MobileIconName = keyof typeof ICONS;

export const iconMarkup = (name: MobileIconName): string =>
  `<svg class="icn" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${ICONS[name]}</svg>`;

/** 将静态标记中的 `data-icon` 属性水合为 lucide SVG。 */
export const hydrateIcons = (): void => {
  document.querySelectorAll<HTMLElement>("[data-icon]").forEach((el) => {
    const name = el.getAttribute("data-icon") as MobileIconName;
    el.innerHTML = el.tagName === "svg" ? ICONS[name] : iconMarkup(name);
  });
};
