...
1. 一台长期运行的付费“轻量应用服务器”或普通 VPS：Ubuntu 22.04/24.04、Debian 11+、CentOS Stream/RHEL/Rocky/AlmaLinux 8+ 等常见 x86_64 系统、至少 1 核 1GB、20GB 硬盘，并明确包含独立公网 IPv4。购买包月或包年实例，不要购买抢占式、竞价、Windows、数据库、GPU、预装面板、自定义 Alpine 镜像或需要另一台设备保活的套餐。
...
2. 在安全组或云防火墙中放行 TCP `22`、`80`、`443` 与 FRP 控制端口（默认 `7000`，可在 Snow 部署表单修改），不要开放隧道端口（默认 `18080`，同样可配置）。
