...
| `browser-wait` | 等待文本出现/消失或等待固定时长 |
| `browser-press-key` | 按下键盘按键（Enter/Tab/Escape/方向键/组合键） |
| `browser-select-option` | 下拉框选择选项（按 value 或 label 匹配） |
| `browser-hover` | 悬停元素（触发 hover 浮层） |
| `browser-upload-file` | 上传文件（CDP 直接注入，无对话框） |
| `browser-back` / `browser-forward` | 浏览器历史后退/前进 |
...
browser-press-key key="a" modifiers=["Control"] instanceId=<id>   # Ctrl+A 全选
