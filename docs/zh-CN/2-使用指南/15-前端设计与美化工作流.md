...
> 仅图片格式。主模型支持视觉时直接使用主模型，否则复用主 API 配置的
> vision 设置（设置 → API → 视觉模型）。
...
| **视觉模型需可用**     | `imagegen-image-describe` 优先用支持视觉的主模型（supportsVision=true）；主模型不支持视觉时依赖主 API 配置中的 vision 通道，未配置时会报错提示 |
