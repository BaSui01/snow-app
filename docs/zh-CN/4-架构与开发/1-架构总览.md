...
| Preload  | Electron contextBridge                 | 将 22 个 API 对象展开为受控的 `window.snow.*`   |
| Main     | Electron 37、TypeScript                | 生命周期、IPC 编排、窗口、PTY/SSH、浏览器、更新 |
...
`src/preload/index.ts` 当前组合 **22 个扁平 API 对象**：`apiConfigApi`、`appLockApi`、`configApi`、`conversationApi`、`workspaceApi`、`sshApi`、`gitApi`、`teamApi`、`systemApi`、`ptyApi`、`windowApi`、`memoApi`、`memoryApi`、`scheduledTaskApi`、`personalizationApi`、`userscriptsApi`、`imageLibraryApi`、`storageApi`、`resourceApi`、`ideApi`、`petApi` 和 `remoteControlApi`。方法直接位于 `window.snow`；类型定义在 `src/preload/types/`。
...
`src/main/app/sessionProxy.ts` 只负责 Electron `session` 网络代理，让 `net.fetch`、webview 和 updater 流量遵循代理设置；它不参与 AI token 或工具事件中转。Rust 侧出站请求（`api/` 的 provider 适配器、`api/models.rs` 模型列表、`mcp/external/http.rs` 的 HTTP MCP 传输）不走 session，而是在 `api/http_client.rs` 统一应用同一份代理配置（含 NO_PROXY 回环 / 内网绕过）。
...
`src/main/ipc/registerIpcHandlers.ts` 是 IPC 注册组的权威清单，当前注册 **25 组**：PTY、native、应用锁、API config、chat、config、conversation、workspace、IDE、SSH、Git、团队、window、notification、memo、memory、定时任务、个性化、image、image library、存储、浏览器密码、用户脚本、宠物、手机远控。`browserNetworkRecorder`、`browserStorageState` 和 `browserTrace` 等是浏览器辅助模块，不应计作注册组。
...
`native/src/storage/database.rs::create_schema` 的直接建表批次创建 29 张表，随后 `image_library::ensure_image_library_table` 建立图像库表，因此当前核心业务表为 **30 张（含 `image_library`）**；代码库索引服务还会建立辅助或按项目动态表。`storage/services/` 当前为 **43 个服务实现模块加 `mod.rs`**。
