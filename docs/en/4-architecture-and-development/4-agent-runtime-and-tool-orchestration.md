...
- **Investigation-phase tool list (template-level dynamic injection, 2026-09-24)**: `native/src/prompt/tool_hints.rs` builds the list from tools **actually callable** in the current project - `lsp-*` (domain scope + usable servers + a matching language stack, the same decision as tool exposure) and `codebase-search` (index available) appear conditionally, while `grep-search` / `filesystem-read` are the constant read-only baseline; at request-build time it replaces the `__ANALYSIS_TOOLS_LINES__` placeholder of the **Plan / Goal / WorkFlow** mode templates (Plan "analysis", Goal "Phase 1 Investigate", WorkFlow "Step 1 pre-design investigation"), so lines for unavailable tools never exist. Tool descriptions carry an additional conditional injection layer (grep-search counter + `lsp-*` timing triggers) - see the LSP design doc §8.10.
