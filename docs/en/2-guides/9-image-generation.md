...
- **Multiple images**: ask for several images in ONE request — the `n`
  parameter accepts 1-8 (default 1): the call internally fans out to `n`
  **concurrent sub-requests** (one image each, because relays/upstreams
  reject `n>1` in a single request) and returns the whole batch at once.
  Pass the **`prompts` array** (1-8 prompt strings) to give every image its
  OWN prompt (e.g. a whole set of different UI mockups in one call), or
  **`requestImages`** to give every request its OWN reference image group
  (restyle several screenshots at once). When the AI fires several tool
...
| ------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prompt`            | string (required) | Generation description, or the edit instruction with reference images (shared by all requests when `prompts` is omitted)                                                                                                                                                                                                                                                                                             |
| `prompts`           | array             | **A different prompt per image**: `["prompt 1", "prompt 2", ...]` (1-8 items; overrides `n`). Each concurrent request uses its own prompt — generate a whole set of DIFFERENT designs in one call                                                                                                                                                                                                                       |
...
| `requestImages`     | array             | **A different reference-image group per request**: `[[group 1...], [group 2...], ...]` (shape same as `images`), group count must equal the request count (= `prompts` length or `n`, 1-8); group i is used by request i — restyle several source images in one call                                                                                                                                                              |
...
| `n`                 | number            | 1-8 (default 1). Generate several images in one call: the call fans out to n **concurrent sub-requests** (one image each — relays/upstreams reject n>1 in a single request), returns the whole batch at once and persists every image. Streaming preview is disabled when n>1. `dall-e-3` always returns 1. Passing `prompts` sets the request count from its length |
