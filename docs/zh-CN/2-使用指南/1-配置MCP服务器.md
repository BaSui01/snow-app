...
5. 保存：只写入配置并立即返回——工具列表先用进程内缓存渲染，随后在后台逐台发现（列表项显示「发现中」），慢服务器不会阻塞保存；
6. 需要重新获取某台服务器的工具时点击该行工具数量按钮；
7. 在项目工具列表中关闭不需要的服务器或单个工具；
8. 用一个只读、最小参数调用验证结果。
...
工具发现结果按服务器缓存在进程内（TTL 60 秒）：命中缓存时列表立即返回；保存、修改或删除某台服务器只失效该服务器的缓存，不会让其他服务器重新连接。服务器与工具的启停属于读取时应用的黑名单，不触发缓存失效。
