...
### 8.9 code-action and execute-command (Deprecated, removed 2026-09-25)

> **[DEPRECATED (2026-09-25)]**: Removed full-stack on 2026-09-25 based on session analytics (0 hits out of 3,761 `lsp-*` calls between 2026-08-14 and 2026-09-25). The cursor-driven interaction model structurally mismatches agent workflows; total tool count reduced from 13 to 11. Re-introduction prerequisite: empirical evidence of agent invocation demand or fundamental UI/interaction redesign, subject to a dedicated proposal review. Historic design below is preserved for archival reference only.

> Background (historical): Phase 4.6 trimmed code-action ("LLM is the completer" — true for **completion**, false for **auto-fix / refactor execution**: the server returns **exact edits** (add import, fix types, add parameters) while LLM-typed fixes often drift off line/column). Phase 5.5 restores code-action and adds the execute-command executor, completing the "fix + refactor execution" quadrant.
...
## Appendix F: Symbol-Name Direct Addressing & Workspace-Global Routing Engine (Phase 8 & Phase 9, 2026-09-25)

### F-1 Background & Motivation

Traditional LSP tool invocations (`lsp-hover`, `lsp-goto`, `lsp-references`, `lsp-rename`, `lsp-call-hierarchy`, etc.) require exact physical `(line, column)` coordinates and explicit `filePath`, forcing AI agents to make prerequisite calls to `lsp-symbols` or `filesystem-read`.
Phase 8 and Phase 9 establish the complete **Symbol Address Resolver & Global Routing Engine (`native/src/mcp/servers/lsp/resolve.rs`)**:
